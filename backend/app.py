...
ACTIVE_COMPILE_PROCESS: subprocess.Popen[str] | None = None
...
async def ingest_url(data: IngestUrlRequest):
    ...
    try:
        result = ingest_urls(
            ...
            same_domain=data.same_domain,
            path_filter=data.path_filter,
            respect_robots=data.respect_robots,
            delay=data.delay,
        )
    except ValueError as e:
        _error_response("INVALID_INGEST_OPTIONS", str(e), 400)

    rc = 0 if not result.failed_urls else 1
    return CommandResponse(
        returncode=rc,
        output=format_ingest_report(result),
    ).model_dump()


@app.post("/api/ingest/url/stream")
async def ingest_url_stream(data: IngestUrlRequest):
    ...
    if data.crawl:
        ...
        if data.path_filter:
            args.extend(["--path-filter", data.path_filter])
        if not data.same_domain:
            args.append("--no-same-domain")
        if not data.respect_robots:
            args.append("--no-respect-robots")
    ...
    def generate():
        ...
        proc.wait()
        output = "\n".join(full_output)
        yield f"data: {json.dumps({'type': 'done', 'returncode': proc.returncode, 'output': output, 'recommendations': []})}\n\n"
...
async def compile_stream(data: CompileRequest):
    ...
    def generate():
        global ACTIVE_COMPILE_PROCESS
        proc = subprocess.Popen(
            cmd, cwd=ROOT, stdout=subprocess.PIPE, stderr=subprocess.STDOUT,
            text=True, bufsize=1,
        )
        ACTIVE_COMPILE_PROCESS = proc
        full_output = []
        for line in proc.stdout:
            stripped = line.rstrip("\n\r")
            full_output.append(stripped)
    ...
    def guarded_generate():
        ...
        try:
            ...
            proc = ACTIVE_COMPILE_PROCESS
            ACTIVE_COMPILE_PROCESS = None
...
# Chat ids are generated by the frontend. Keep them to characters that cannot
# escape kb/chats/ or mean anything special to the filesystem.
_CHAT_ID_CHARS = set("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-_")


def chat_path(chat_id: str) -> Path:
    if not chat_id or not set(chat_id) <= _CHAT_ID_CHARS:
        _error_response("INVALID_CHAT_ID", f"Invalid chat id: {chat_id}", 400)
...
async def list_chats():
    ...
    chats = []
    for p in sorted(CHATS.glob("*.json")):
        try:
...
async def save_chat(chat_id: str, chat: Conversation):
    if chat.id != chat_id:
        _error_response("ID_MISMATCH", "Body id does not match the URL id", 400)
    path = chat_path(chat_id)
    try:
        CHATS.mkdir(parents=True, exist_ok=True)
        save_json(path, chat.model_dump())
    except OSError as e:
        _error_response("SAVE_FAILED", str(e))
    return {"success": True}
...
async def delete_chat(chat_id: str):
    path = chat_path(chat_id)
    if not path.exists():
        _error_response("NOT_FOUND", "Chat not found", 404)
    try:
        path.unlink()
    except OSError as e:
        _error_response("DELETE_FAILED", str(e))
