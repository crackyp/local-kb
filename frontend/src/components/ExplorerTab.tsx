...
function InlineConfirm({ message, confirmLabel, confirmVariant, onConfirm, onCancel }: {
  ...
  return (
    <div ref={ref} role="dialog" aria-modal="true" tabIndex={-1}
      className="absolute right-0 top-full mt-1 z-50 bg-white border border-zinc-200 rounded shadow-lg p-3 min-w-[220px] transition-colors duration-150 ease-out">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
          ...
          {trashItems.map((item, idx) => (
            <div key={`${item.category}-${item.original_name}-${idx}`}
              className="flex items-center justify-between p-3 rounded border border-zinc-100 hover:bg-zinc-50 transition-colors duration-150 ease-out">
    ...
    <>
      <div className="flex flex-col h-[calc(100vh-8rem)] bg-white rounded shadow-sm border border-zinc-200 overflow-hidden transition-colors duration-150 ease-out">
        ...
        <div className="flex flex-wrap items-center gap-2 px-4 py-2 border-b border-zinc-200 bg-white flex-shrink-0 md:flex-nowrap md:gap-3">
          <div className="flex items-center gap-0.5 bg-zinc-100 rounded p-0.5">
            {CATEGORIES.map((tab) => (
              <button key={tab} onClick={() => switchTab(tab)}
                className={`h-7 px-3 inline-flex items-center gap-1.5 text-xs font-medium rounded transition-colors duration-150 ease-out ${
          ...
          {activeTab === "wiki" && (
            <div className="flex items-center gap-0.5 bg-zinc-100 rounded p-0.5 flex-shrink-0">
              {([
                ...
                <button key={mode} onClick={() => setWikiView(mode)}
                  ...
                  className={`h-7 w-7 inline-flex items-center justify-center rounded transition-colors duration-150 ease-out ${
            ...
            <div className="flex items-center gap-1 overflow-x-auto flex-1 min-w-0">
              {filterChip && (
                <button onClick={() => setFilterChip("")}
                  className="h-6 px-2 inline-flex items-center gap-1 text-[10px] font-medium rounded bg-zinc-200 text-zinc-600 hover:bg-zinc-300 whitespace-nowrap flex-shrink-0 transition-colors duration-150 ease-out">
                  <X className="w-3 h-3" /> clear
                </button>
              )}
              {extensionCounts.slice(0, 6).map(([ext, count]) => (
                <button key={ext} onClick={() => setFilterChip(filterChip === ext ? "" : ext)}
                  className={`h-6 px-2 inline-flex items-center gap-1 text-[10px] font-medium rounded whitespace-nowrap flex-shrink-0 transition-colors duration-150 ease-out ${
                    filterChip === ext
                ...
                <div className="p-6 text-center">
                  ...
                  {active.error && !filter && (
                    <button onClick={handleRefresh}
                      className="px-4 py-2 text-xs font-medium rounded bg-violet-600 text-white hover:bg-violet-500 transition-colors duration-150 ease-out">
                      Retry
                    </button>
                  )}
      ...
      <div className="fixed bottom-4 right-4 z-50 flex flex-col gap-2" role="status" aria-live="polite">
        {toasts.map((toast) => (
          <div
            key={toast.id}
            className={`px-4 py-3 rounded shadow-lg text-sm flex items-center gap-3 min-w-[280px] max-w-md transition-all ${
              toast.type === "error"
